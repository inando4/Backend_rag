...
class RAGService:
    def __init__(self):
        ...
        # [CONFIG] Configuración LLM (SOLO LOCAL)
        ...
        logger.info(f"[OK] RAG Service iniciado")
    ...
    def keyword_search(self, query, documents):
        ...
        synonyms = {
            ...
            # [INFO] NUEVOS SINÓNIMOS PARA CONSECUENCIAS
        ...
        # [INFO] NUEVO: Detectar preguntas sobre CONSECUENCIAS de NO matricularse
        ...
        for i, doc in enumerate(documents):
            ...
            # [INFO] LÓGICA ESPECIAL PARA PREGUNTAS SOBRE CONSECUENCIAS
            if consequence_question:
                ...
                if matches >= 2:  # Al menos 2 keywords para match fuerte
                    ...
                    logger.info(f"    [MATCH] Consecuencias encontradas en {doc.get('id_chunk')}: {matches} keywords")
                elif matches == 1:
                    score = 1500  # Score medio si tiene solo 1 keyword
                    logger.info(f"    [MATCH] Parcialmente relevante en {doc.get('id_chunk')}: {matches} keyword")
                else:
